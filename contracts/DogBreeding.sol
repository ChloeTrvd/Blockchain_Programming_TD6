...
contract DogBreeding is DogOwnership {
    ...
    /// @param _kennel A reference to the Kitty in storage which needs its timer started.
    function _triggerCooldown(Dog storage _kennel) internal {
        // Compute an estimation of the cooldown time in blocks (based on current cooldownIndex).
        _kennel.cooldownEndBlock = uint64((cooldowns[_kennel.cooldownIndex]/secondsPerBlock) + block.number);

        // Increment the breeding count, clamping it at 13, which is the length of the
        // cooldowns array. We could check the array size dynamically, but hard-coding
        // this as a constant saves gas. Yay, Solidity!
        if (_kennel.cooldownIndex < 13) {
            _kennel.cooldownIndex += 1;
        }
    }
    ...
    {
        require(_dogId > 0);
        Dog storage stock = kennel[_dogId];
        return _isReadyToBreed(stock);
    }
        ...
        returns (bool)
    {
        ...
        return kennel[_dogId].siringWithId != 0;
    }
    ...
    function _canBreedWithViaAuction(uint256 _matronId, uint256 _sireId)
        ...
        returns (bool)
    {
        Dog storage matron = kennel[_matronId];
        Dog storage sire = kennel[_sireId];
    ...
    function _breedWith(uint256 _matronId, uint256 _sireId) internal {
        ...
        emit Pregnant(dogIndexToOwner[_matronId], _matronId, _sireId, matron.cooldownEndBlock);
    }
        ...
        returns(uint256)
    {
        ...
        Dog storage sire = kennel[sireId];
        ...
        uint256 dogId = _createDog(_matronId, matron.siringWithId, parentGen + 1, childGenes, owner);
